...
def fetch_notices(base: date, lookback_days: int = LOOKBACK_DAYS) -> dict:
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        if stop:
            break
        time.sleep(0.05 if os.environ.get("LAW_FETCH_FAST") or os.environ.get("CI") else 0.2)
...
def main() -> None:
    ...
    if CACHE_PATH.is_file():
        try:
            ...
            prev_notices = prev.get("notices") or []
            prev_latest = prev_notices[0]["date"] if prev_notices else None
        except Exception:  # noqa: BLE001
            prev_count = 0

    # 자율 검증: MOEL 게시판 HTML 구조가 바뀌는 등의 이유로 파싱이 깨지면
    # 0건(또는 비정상적으로 적은 건수)이 수집되기 쉽다. 이 경우 예전 데이터를
    # "0건"으로 덮어써 버리면 사이트에서 입법예고가 통째로 사라지므로,
    # 그런 상황에서는 저장을 건너뛰고 직전 캐시를 그대로 유지한다.
    reasons: list[str] = []
    if payload.get("errors"):
        reasons.append(f"fetch_errors:{payload['errors']}")
    if payload["count"] == 0:
        reasons.append("zero_notices_collected")
    elif prev_count and payload["count"] < prev_count * 0.3:
        # 이전 대비 70% 이상 급감 = 파싱 누락 가능성이 높음(정상적으로 게시물이
        # 줄어드는 경우는 드묾 — lookbackDays=90 이므로 자연 감소보다 파싱 실패가 더 흔함)
        reasons.append(f"count_dropped_sharply:{prev_count}->{payload['count']}")

    ok = not reasons
    payload["ok"] = ok
    payload["prevCount"] = prev_count
    ...
    if ok:
        ...
        path = CACHE_PATH
        print(f"::error::notices refresh rejected ({reasons}) - 직전 캐시 유지(count={prev_count})")
